...
extension MockAIService {
    ...
    static func medicineInfoMock() -> MockAIService {
        let mock = MockAIService()
        mock.mockResponse = """
        **药物信息查询**

        **💊 药物概述**
        阿司匹林（乙酰水杨酸）是一种非甾体抗炎药（NSAID），常用于退热、镇痛及抗血小板治疗。

        **⚠️ 药物相互作用**
        - **高风险**：与华法林合用显著增加出血风险，需密切监测 INR
        - **中等风险**：与布洛芬合用可能降低阿司匹林的心脏保护作用
        - **注意**：与 ACE 抑制剂合用可能降低其降压效果

        **📋 服用建议**
        - 饭后服用以减少胃肠道刺激
        - 心脏保护剂量（75-100mg）通常每日一次
        - 避免与酒精同服

        **🚨 注意事项**
        - 对 NSAIDs 过敏者禁用
        - 活动性消化道溃疡患者慎用
        - 儿童及青少年发热禁用（Reye 综合征风险）

        *以上信息仅供参考，请在医生或药剂师指导下用药。*
        """
        return mock
    }
}
