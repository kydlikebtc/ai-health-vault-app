...
// MARK: - 空实现占位（供 Preview 编译）

struct MedicationListView: View {
    let member: Member
    var body: some View {
        List(member.medications, id: \.id) { med in
            VStack(alignment: .leading) {
                Text(med.name).font(.headline)
                Text("\(med.dosage) · \(med.frequency.displayName)").font(.caption).foregroundStyle(.secondary)
            }
        }
...
struct CheckupListView: View {
    let member: Member
    var body: some View {
        List(member.checkups, id: \.id) { report in
            VStack(alignment: .leading) {
                Text(report.reportTitle).font(.headline)
                Text(report.hospitalName).font(.caption).foregroundStyle(.secondary)
            }
        }
...
struct VisitListView: View {
    let member: Member
    var body: some View {
        List(member.visits, id: \.id) { visit in
            VStack(alignment: .leading) {
                Text(visit.diagnosis.isEmpty ? "就诊记录" : visit.diagnosis).font(.headline)
                Text(visit.hospitalName).font(.caption).foregroundStyle(.secondary)
            }
        }
...
struct WearableListView: View {
    let member: Member
    var body: some View {
        List(member.wearableData, id: \.id) { entry in
            HStack {
                Image(systemName: entry.metricType.icon)
                    .foregroundStyle(.red)
                VStack(alignment: .leading) {
                    Text(entry.metricType.displayName).font(.headline)
                    Text(entry.displayValue).font(.caption).foregroundStyle(.secondary)
                }
            }
        }
...
struct MedicalHistoryListView: View {
    let member: Member
    var body: some View {
        List(member.medicalHistory, id: \.id) { history in
            VStack(alignment: .leading) {
                Text(history.conditionName).font(.headline)
                Text(history.isChronic ? "慢性病" : "已痊愈").font(.caption).foregroundStyle(.secondary)
            }
        }
...
struct DailyLogListView: View {
    let member: Member
    var body: some View {
        List(member.dailyTracking, id: \.id) { log in
            HStack {
                Text(log.mood.emoji).font(.title2)
                VStack(alignment: .leading) {
                    Text(log.date.formatted(date: .abbreviated, time: .omitted)).font(.headline)
                    Text("睡眠 \(String(format: "%.1f", log.sleepHours))h · 运动 \(log.exerciseMinutes)min")
                        .font(.caption).foregroundStyle(.secondary)
                }
            }
        }
        .navigationTitle("日常日志")
    }
}
